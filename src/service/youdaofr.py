...
class Youdaofr(WebService):
    ...
    def _get_singledict(self, single_dict, lang="fr"):
        ...
        try:
            result = urllib2.urlopen(url, timeout=5).read()
            return (
                '<div id="%s_contentWrp" class="content-wrp dict-container"><div id="%s" class="trans-container %s ">%s</div></div><div id="outer"><audio id="dictVoice" style="display: none"></audio></div>'
                % (single_dict, single_dict, single_dict, result)
            )
        except:
            return ""
