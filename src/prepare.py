...
from .query import (
    ...
    query_from_editor_current_field,
)
...
def setup_browser_menu():
    def on_setup_menus(browser):
        ...
        action_queryselected.triggered.connect(query_decor(query_from_browser, browser))
...
def setup_context_menu():
    def on_setup_menus(web_view, menu):
        ...
        action1.triggered.connect(
            query_decor(query_from_editor_all_fields, web_view.editor)
        )
        action2.triggered.connect(
            query_decor(query_from_editor_current_field, web_view.editor)
        )
...
def customize_addcards():
    AddCards.setupButtons = wrap(AddCards.setupButtons, add_query_button, "before")
